...
$items_total_count = ($search=='') || !isset($_GET['search']) ? count($courses->find_by_query("SELECT * FROM course WHERE department IN (SELECT name FROM department WHERE faculty='{$user->faculty}')")) : count($courses::find_by_query($sql));
...
$sql = ($search='') || !isset($_GET['search']) ? "SELECT * FROM course WHERE department IN (SELECT name FROM department WHERE faculty='{$user->faculty}') ORDER BY id DESC" : $sql;
