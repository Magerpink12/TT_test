...
$items_total_count = ($search=='') || !isset($_GET['search']) ? count($venues->find_by_query("SELECT * FROM venue WHERE department IN (SELECT name FROM department WHERE faculty='{$user->faculty}') AND department='{$user->department}'")) : count($venues::find_by_query($sql));
...
$sql = ($search=='') || !isset($_GET['search']) ? "SELECT * FROM venue WHERE department IN (SELECT name FROM department WHERE faculty='{$user->faculty}') AND department='{$user->department}' ORDER BY id DESC" : $sql;
