...
if (!isset($session->user['dept_admin']['user_id'])) {
	header('Location: ../');
}
$user = Coordinator::find_by_id($session->user['dept_admin']['user_id']);
if (!$user) {
	header('Location: ../');
}
...
if (isset($_GET['page']) && isset($_GET['search'])) {
	...
	switch ($_GET['page']) {
		...
		OR `department` LIKE '%" . $_GET['search'] . "%') 
		AND department IN 
		(SELECT name FROM department WHERE faculty='{$user->faculty}') AND department='{$user->department}' 
		ORDER BY id DESC";
			break;
		case 'courses':
		...
		<ul class="navbar-nav navbar-align ">
			<li class="nav-item dropdown">
				...
				<a class="nav-icon dropdown-toggle text-light" href="#" id="alertsDropdown" data-toggle="dropdown">
					<div class="position-relative">
						...
						<span class="indicator"><?php echo 0; ?></span>
					</div>
				</a>
			...
			<li class="nav-item dropdown">
				...
				<a class="nav-link dropdown-toggle d-none d-sm-inline-block text-light" href="#" data-toggle="dropdown">
					<img style="border: 1px solid gray;" src="img/avatars/avatar.jpg" class="avatar img-fluid rounded mr-1" alt="Charles Hall" /> <span class="text-light"><?php echo $user->name ?> (Department Admin)</span>
				</a>
